...
func NewIdWorker(workerId, datacenterId int64, twepoch int64) (*IdWorker, error) {
	...
	if workerId > maxWorkerId || workerId < 0 {
		logger.LogSugar.Errorf("worker Id can't be greater than %d or less than 0", maxWorkerId)
		return nil, fmt.Errorf("worker Id: %d error", workerId)
	}
	if datacenterId > maxDatacenterId || datacenterId < 0 {
		logger.LogSugar.Errorf("datacenter Id can't be greater than %d or less than 0", maxDatacenterId)
	...
	//logger.LogSugar.Infof("worker starting. timestamp left shift %d, datacenter id bits %d, worker id bits %d, sequence bits %d, workerid %d", timestampLeftShift, datacenterIdBits, workerIdBits, sequenceBits, workerId)
	return idWorker, nil
}
...
func (id *IdWorker) NextId() (int64, error) {
	...
	if timestamp < id.lastTimestamp {
		logger.LogSugar.Errorf("clock is moving backwards.  Rejecting requests until %d.", id.lastTimestamp)
...
func (id *IdWorker) NextIds(num int) ([]int64, error) {
	if num > maxNextIdsNum || num < 0 {
		logger.LogSugar.Errorf("NextIds num can't be greater than %d or less than 0", maxNextIdsNum)
