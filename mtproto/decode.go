...
func (m *DecodeBuf) Uint16() uint16 {
	if m.err != nil {
		return 0
	}
	if m.off+2 > m.size {
		m.err = errors.New("DecodeUint16")
		return 0
	}
...
func (m *DecodeBuf) StringBytes() []byte {
	...
	if m.off+size > m.size {
		m.err = errors.New(fmt.Sprintf("DecodeStringBytes: Wrong size, m.off:%d+size:%d m.size:%d", m.off, size, m.size))
		return nil
	}
	x := make([]byte, size)
	copy(x, m.buf[m.off:m.off+size])
...
func (m *DecodeBuf) Object() (r TLObject) {
	...
	if r == nil {
		logger.LogSugar.Errorf("Can't find registed classId: %x", uint32(classID))
		return nil
	}

	err := r.(TLObject).Decode(m)
	if err != nil {
		logger.LogSugar.Errorf("Object classId:%d decode err:%v m.buf:%v", classID, err, m.buf)
		return nil
	}
	return
}
